...
import { buildMovePreview, moveFiles } from "@/lib/file-mover";
...
export async function POST(request: Request) {
  try {
    ...
    if (dryRun) {
      const preview = buildMovePreview(files);
      return NextResponse.json({ preview, totalFiles: files.length });
    }

    const result = moveFiles(files, resolved);
